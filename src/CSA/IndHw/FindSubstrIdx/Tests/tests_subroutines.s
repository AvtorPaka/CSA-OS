...
main:
	...
	for_test_loop:
		...
		# Вызов подпрограммы записи данных файла в память для теста
		mv a0 s9
		mv a1 s3
		la a2 data_buffer
		li a3 BUFFER_SIZE
		...
		# Вызов подпрограммы для поиска индексов
		mv a0 s2
		mv a1 s8
		jal find_substr_idx
		...
		if_not_empty_array:
			...
			# Вызов подпрограммы перевода массива 32-битных чисел-индексов в строку
			mv a0 s1
			mv a1 s0
			...
			# Вызов подпрограммы записи полученных данных в виде строки в файл
			mv a0 s7
			mv a1 s3
			mv a2 s1
			mv a3 s0
