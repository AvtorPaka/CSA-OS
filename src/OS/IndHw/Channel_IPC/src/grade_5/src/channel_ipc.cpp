...
int main(int argc, char* argv[]) {
    ...
    char fifoReadProcessName[] = "rp.fifo";
    char fifoProcessWriteName[] = "pw.fifo";
    ...
    mknod(fifoReadProcessName, S_IFIFO | 0666, 0);
    mknod(fifoProcessWriteName, S_IFIFO | 0666, 0);
    ...
    if (ch_count_id < 0) {
        ...
        int fifoRPD = OpenFifoWithCheck(fifoReadProcessName, O_WRONLY);
        ...
        writeChannelBytes = write(fifoRPD, asciiBuffer, readBytes);
        if (writeChannelBytes != readBytes) {
            fprintf(stderr, "Reading process: Unable to write all bytes to rp FIFO\n");
            exit(1);
        }

        if (close(fifoRPD) < 0) {
            fprintf(stderr, "Reading process: Unable to close rp FIFO\n");
            exit(1);
        }
        ...
        if (ch_write_id < 0) {
            ...
            int fifoRPD = OpenFifoWithCheck(fifoReadProcessName, O_RDONLY);
            ...
            while ((readChannelBytes = read(fifoRPD, processingSlidingBuffer, PROCESSING_BUFFER_SIZE)) > 0) {
            ...
            if (readChannelBytes < 0) {
                fprintf(stderr, "Processing process: Unable to read from rp FIFO\n");
                exit(1);
            }

            if (close(fifoRPD) < 0) {
                fprintf(stderr, "Processing process: Unable to close rp FIFO\n");
                exit(1);
            }
            ...
            int fifoPWD = OpenFifoWithCheck(fifoProcessWriteName, O_WRONLY);
            ssize_t actualWriteSize;
            actualWriteSize = write(fifoPWD, resultString, writeSize);

            if (actualWriteSize != writeSize) {
                fprintf(stderr, "Processing process: Unable to write all bytes to pw FIFO\n");
                exit(1);
            }

            if (close(fifoPWD) < 0) {
                fprintf(stderr, "Processing process: Unable to close writing side of pw FIFO\n");
                exit(1);
            }
            ...
            int fifoPWD = OpenFifoWithCheck(fifoProcessWriteName, O_RDONLY);

            ssize_t readChannelBytes;
            readChannelBytes = read(fifoPWD, processedInfoBuffer, BUFFER_SIZE);
            if (readChannelBytes < 0) {
                fprintf(stderr, "Reading: Unable to read from pw FIFO.\n");
                exit(1);
            }

            if (close(fifoPWD) < 0) {
                fprintf(stderr, "Writing process: Unable to close pw FIFO\n");
                exit(1);
            }
