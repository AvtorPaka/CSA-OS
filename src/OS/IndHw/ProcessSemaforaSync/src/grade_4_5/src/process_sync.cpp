...
void philosopherProcess() {
    ...
    while (true) {
        ...
        std::cout << '[' << processPid << ']' << " | " << "Trying to grab left fork " << left_fork_name <<  '\n';
        sem_wait(left_fork);
        has_left_fork = true;
        std::cout << '[' << processPid << ']' << " | " << "Grab left fork " << left_fork_name << '\n';

        // Получение правой вилки
        std::cout << '[' << processPid << ']' << " | " << "Trying to grab right fork " << right_fork_name << '\n';
        sem_wait(right_fork);
        has_right_fork = true;
        std::cout << '[' << processPid << ']' << " | " << "Grab right fork " << right_fork_name << '\n';
        ...
        sem_post(left_fork);
        has_left_fork = false;
        std::cout << '[' << processPid << ']' << " | " << "Release left fork " << left_fork_name <<  '\n';
        sem_post(right_fork);
        has_right_fork = false;
        std::cout << '[' << processPid << ']' << " | " << "Release right fork " << right_fork_name <<  '\n';
        sem_post(forkWaiter);
