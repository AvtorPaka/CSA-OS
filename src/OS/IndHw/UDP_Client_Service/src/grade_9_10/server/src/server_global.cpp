...
void cleanup_philosopher_semaphores_if_held(philosopher_state_t* p_state) {
    ...
    if (p_state->has_right_fork) {
        ...
        snprintf(broadcast_msg, sizeof(broadcast_msg), "[Srv for P%d] (Thread) Released right fork %s.", p_id, sem_name);
    ...
    if (p_state->has_left_fork) {
        ...
        print_log("Server", -1, log_buffer);
        snprintf(broadcast_msg, sizeof(broadcast_msg), "[Srv for P%d] (Thread) Released left fork %s.", p_id, sem_name);
        broadcast_to_loggers(broadcast_msg);
    }
    if (p_state->has_waiter_permission) {
        ...
        print_log("Server", -1, log_buffer);
        snprintf(broadcast_msg, sizeof(broadcast_msg), "[Srv for P%d] (Thread) Released waiter permission (%s).", p_id, WAITER_SEM_NAME);
        broadcast_to_loggers(broadcast_msg);
    }
