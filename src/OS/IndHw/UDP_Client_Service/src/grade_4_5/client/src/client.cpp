    ...
    if (sig == SIGINT || sig == SIGTERM) {
        if (!client_shutdown_flag) {
            ...
            if (philosopher_id_global != -1) {
                 char log_msg_sig[100];
                 snprintf(log_msg_sig, sizeof(log_msg_sig), "P%d: SIGINT/SIGTERM received. Initiating shutdown...", philosopher_id_global);
...
int send_message_to_server(int sock_fd, const char* message) {
    ...
    if (send(sock_fd, msg_buffer_send, strlen(msg_buffer_send), 0) < 0) {
        char error_log[150];
...
ssize_t receive_message_from_server(int sock_fd, char* buffer, size_t buffer_len) {
    ...
    if (n_read < 0) {
        ...
        print_log("Client", philosopher_id_global, "recv() returned 0 bytes (empty datagram received). Unexpected.");
    }
    if (n_read > 0) {
        buffer[n_read] = '\0';
...
int main(int argc, char *argv[]) {
    ...
    if (n_read <= 0) {
        if (n_read == -2) { 
            ...
            snprintf(log_buffer, sizeof(log_buffer), "Error receiving JOIN response (n_read=%zd, errno=%d).", n_read, errno);
            print_log("Client", -1, log_buffer);
        }
        goto client_cleanup_exit;
    }
    ...
    while (!client_shutdown_flag) {
        ...
        if (n_read <= 0) {
            if (n_read == -2) {
                print_log("Client", philosopher_id_global, "Timeout waiting for GRANTED. Server might be down or busy.");
                 ...
                 snprintf(log_buffer, sizeof(log_buffer), "Error or no response for GRANTED (n_read=%zd, errno=%d).", n_read, errno);
                 print_log("Client", philosopher_id_global, log_buffer);
            }
            client_shutdown_flag = 1; 
            break;
        ...
        if (n_read <= 0) {
            if (n_read == -2) {
                print_log("Client", philosopher_id_global, "Timeout waiting for OK after RELEASE. Assuming forks released.");
            } else {
                snprintf(log_buffer, sizeof(log_buffer), "Error or no response for OK (n_read=%zd, errno=%d).", n_read, errno);
                print_log("Client", philosopher_id_global, log_buffer);
            }
            client_shutdown_flag = 1; 
            break;
...
client_cleanup_exit:
    if (client_shutdown_flag && has_forks_currently && philosopher_id_global != -1) {
    ...
    if (philosopher_id_global != -1) {
        ...
        if (n_read > 0 && strncmp(buffer, MSG_SERVER_DOWN, strlen(MSG_SERVER_DOWN)) == 0) {
            send_leave = 0; 
            print_log("Client", philosopher_id_global, "Not sending LEAVE because SERVER_DOWN was received.");
        }
        
        if (client_shutdown_flag && send_leave) {
